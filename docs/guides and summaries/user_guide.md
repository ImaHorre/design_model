...
- `map_Q_spread_pct.png` — flow spread across rungs (high = non-uniform; aim < 10%)
- `map_dP_spread_pct.png` — driving-force spread across rungs (high = non-uniform)
- `map_P_peak_Pa.png` — peak pressure (delamination risk)
- `map_f_mean.png` — mean droplet frequency [Hz]
- `map_dP_avg_mbar.png` — mean rung ΔP [mbar]
- `map_Q_rung_nL_hr.png` — mean flow per rung [nL/hr]
...
**Frequency predictions look right but spread is high**

A steep pressure drop along the oil rail means early rungs get more flow. The fix is to increase main channel cross-section (Mcd, Mcw) relative to rung resistance, or reduce the number of rungs. The `dP_spread_pct` metric quantifies this — aim for < 10%.
